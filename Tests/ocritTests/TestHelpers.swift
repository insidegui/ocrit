...
public enum ExpectedOutput: Hashable {
    public enum Operator: Hashable {
        case equal(String)
        ...
        var expectedValue: String {
            switch self {
            case .equal(let v), .notEqual(let v), .contain(let v), .notContain(let v):
                return v
            }
        }
    }

    case stderr(Operator)
    case stdout(Operator)
}
...
extension XCTest {
    ...
    func validateOutput(stderr: String, stdout: String, expected: Set<ExpectedOutput>, file: StaticString, line: UInt) {
        func validate(value: String, op: ExpectedOutput.Operator) {
            let sanitizedValue = value.trimmingCharacters(in: .whitespacesAndNewlines)
            let expectedValue = op.expectedValue
            let sanitizedExpectedValue = expectedValue.trimmingCharacters(in: .whitespacesAndNewlines)

            switch op {
            case .equal:
                if sanitizedValue != sanitizedExpectedValue {
                    XCTFail("\"\(value)\" is not equal to \"\(expectedValue)\"",
                            file: (file), line: line)
                }
            case .notEqual:
                if sanitizedValue == sanitizedExpectedValue {
                    XCTFail("\"\(value)\" is equal to \"\(expectedValue)\"",
                            file: (file), line: line)
                }
            case .contain:
                if !sanitizedValue.localizedCaseInsensitiveContains(sanitizedExpectedValue) {
                    XCTFail("\"\(value)\" does not contain \"\(expectedValue)\"",
                            file: (file), line: line)
                }
            case .notContain:
                if sanitizedValue.localizedCaseInsensitiveContains(sanitizedExpectedValue) {
                    XCTFail("\"\(value)\" contains \"\(expectedValue)\"",
                            file: (file), line: line)
                }
            }
        }
        ...
        for expect in expected {
            switch expect {
            case .stderr(let op):
                validate(value: stderr, op: op)
            case .stdout(let op):
                validate(value: stdout, op: op)
            }
        }
    }
    ...
    {
        try AssertExecuteCommand(command: command, expected: [expected], exitCode: exitCode, file: file, line: line)
    }

    public func AssertExecuteCommand(
        command: String,
        expected: Set<ExpectedOutput>? = nil,
        exitCode: ExitCode = .success,
        file: StaticString = #file, line: UInt = #line) throws
    {
        ...
        if let expected = expected {
            validateOutput(stderr: errorActual, stdout: outputActual, expected: expected, file: file, line: line)
        }
